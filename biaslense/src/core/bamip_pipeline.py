...
class BAMIPPipeline:
    ...
    def _generate_improved_response(self, prompt: str, ai_response: str,
        ...
        try:
            ...
            # Get OpenAI API key from environment variables (works in REST API, Streamlit, local)
            # Falls back to Streamlit secrets for backwards compatibility
            ...
            if not openai_key:
                # Gracefully skip improved response generation if no API key available
                return ai_response + "\n\n*Note: Improved response generation unavailable (OpenAI API key not configured).*"
