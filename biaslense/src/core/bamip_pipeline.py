...
class BAMIPPipeline:
    ...
    def _generate_improved_response(self, prompt: str, ai_response: str,
        ...
        try:
            ...
            if not anthropic_key:
                return ai_response + "\n\n*Note: Improved response generation unavailable (Anthropic API key not configured).*"

            client = anthropic.Anthropic(api_key=anthropic_key)
            model_info = f" (Generated by {ai_model.value})" if ai_model != AIModel.UNKNOWN else ""

            if risk_level == RiskLevel.LOW:
                user_content = f"Generate a response that is generally balanced and free of bias. The original response is: {ai_response}"
                ...
                user_content = f"Generate a response that is free of bias and provides more balanced information. The original response is: {ai_response}. The mitigated response is: {improved}"
                ...
                user_content = f"Completely rewrite this response to eliminate bias and provide accurate, balanced information. The original response is: {ai_response}. The mitigated response is: {mitigated}"
            ...
            model_info = f" (Generated by {ai_model.value})" if ai_model != AIModel.UNKNOWN else ""
            error_note = f"\n\n*[Debug] Improved response error: {type(e).__name__}: {e}*"
            if risk_level == RiskLevel.LOW:
                return f"{ai_response}\n\n*Note: This response has been reviewed for bias and found to be generally balanced.{model_info}*" + error_note
            elif risk_level == RiskLevel.MEDIUM:
                improved = mitigation_result.mitigated_text
                return f"{improved}\n\n*This response has been improved to reduce potential bias and provide more balanced information.{model_info}*" + error_note
            else:  # HIGH risk
                mitigated = mitigation_result.mitigated_text
                return f"{mitigated}\n\n*This response has been substantially rewritten to eliminate bias and provide more accurate information.{model_info}*" + error_note
